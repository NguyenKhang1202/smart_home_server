...
const register = async (req, res, next) => {
  ...
  const user = await createUserDb({ email, username, password: hashedPw, fullName });
...
const verifyCodeUser = async (req, res, next) => {
  ...
  await Promise.all([user.save(), sendEmail(email, 'New password on Smart Home', 'This is your new password', newPassword)])
